...
class CGIengine:
    ...
    def drawTriangles(self, vertex_data, color_data, index_data, modelT, normT):
        for i in range(0, len(index_data), 3):
            idx0 = index_data[i]
            idx1 = index_data[i + 1]
            idx2 = index_data[i + 2]

            # transform vertices by applying the transformation matrices
            ...
            # Convert normalized coordinates to screen coordinates using the view window
            ...
            self.rasterizeTriangle(v0, v1, v2)
    ...
    def rasterizeTriangle(self, p0, p1, p2):
        ...
        # Add a small tolerance to handle small triangles and avoid degenerate cases
        if abs(area) < 1e-5:
            return  # Degenerate triangle, skip

        print(f"Rasterizing triangle with vertices: {p0}, {p1}, {p2}")
        ...
        for x in range(min_x, max_x + 1):
            for y in range(min_y, max_y + 1):
                ...
                if w0 >= 0 and w1 >= 0 and w2 >= 0:
                    ...
                    print(f"Setting pixel at ({x}, {y}) with color ({r}, {g}, {b})")
